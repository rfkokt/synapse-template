...
const toolsPkg = JSON.parse(fs.readFileSync(toolsPkgPath, 'utf8'));
...
console.log(`   🔧 Using tools package: ${TOOLS_SCOPE}`);
try {
  execSync(`pnpm nx g ${TOOLS_SCOPE}:mfe ${mfeName} --port=${port}`, {
    cwd: MONOREPO_ROOT,
    stdio: 'inherit',
  });
} catch {
...
// Extract scope from tools package (e.g., @synapse from @synapse/tools)
